...
def evaluate_args(_func=None, *, dest_arg=None):

    def decorator_args(func):
        @wraps(func)
        def func_wrapper(self, *args, **kwargs):
            # print(f'{func.__name__} {args}')
            args = self._evaluate_args(dest_arg, args)
...
class Q1Core:
    ...
    def _evaluate_args(self, dest_arg, args):
        args = list(args)
        for i,arg in enumerate(args):
            c = arg[0]
            if i == dest_arg:
                if c != 'R':
                    raise Exception('Expected register argument')
                reg_nr = int(arg[1:])
                args[i] = reg_nr
            elif c == '@':
                try:
                    ...
                    raise Exception(f'Label {arg} not defined')
            elif c == 'R':
